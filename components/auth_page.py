...
def create_auth_layout():
    return html.Div(className='auth-page', children=[
        html.Div(className='auth-bg', children=[
            ...
            html.Div(className='bubbles', children=[html.Span(className='bubble') for _ in range(12)]),

            # relatable sentence above the card
            html.Div(className='auth-sentence',
                     children='Protect your future — check your neurological risk in minutes.'),

            # existing login/register card (keep your current card markup)
